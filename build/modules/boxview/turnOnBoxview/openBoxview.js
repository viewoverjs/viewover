...
import { nextButton, previousButton, fullscreenEntryButton, fullscreenExitButton, thumbnailsShowButton, thumbnailsHideButton, boxviewThumbnailsTrack, boxviewMainContent, boxviewMediaWrapper, fullscreenButtons, thumbnailsButtons } from '../../document/docConstants.js';
...
import { setScrollbarHeight, scrollThumbnailToViewport } from '../thumbnailsTrack/handleThumbnailsOverflow.js';
...
export const openBoxview = async (e, settings) => {
  ...
  const mediaElementsTypes = checkMediaElementsTypes(mediaElements.preparedMediaElements);
  ...
  if (settings.thumbnails == true) {
    ...
    const currentThumbnail = thumbnailsTrackList.find(thumbnailWrapper => thumbnailWrapper.querySelector('.boxview__thumbnail').getAttribute('data-boxview-thumbnail-src') === elementTargetSrc);
