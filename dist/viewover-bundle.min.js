...
var C = (e, t, i) => (re(e, t, "read from private field"), i ? i.call(e) : t.get(e)), y = (e, t, i) => {
  if (t.has(e))
...
`, B = document.createElement("dialog");
B.setAttribute("id", "viewover");
B.classList.add("viewover");
B.innerHTML = _e;
...
function ze(e, t, i) {
  for (let n = 0; n < e.length; n++) {
    const o = e[n];
    t.includes(o.localName) && (N = o.localName), o.localName === "div" && xe(o, t), i.includes(N) || i.push(N);
  }
}
function ke(e) {
  let t = new Array();
...
const m = document.getElementById("control-bar"), Ee = document.getElementById("nav-next-button"), Le = document.getElementById("nav-previous-button"), u = document.getElementById("zoom-in-button"), p = document.getElementById("zoom-out-button"), Se = document.getElementById("close-button"), Me = document.getElementById("fullscreen-buttons"), W = document.getElementById(
  "fullscreen-entry-button"
), Q = document.getElementById(
  "fullscreen-exit-button"
), h = document.getElementById("viewover-container"), ae = document.getElementById("viewover-content"), x = document.getElementById(
  "viewover-main-content"
), L = document.getElementById("viewover-media"), v = document.getElementById(
  "viewover-thumbnails-track"
), ee = "76px", Be = document.getElementById("thumbnails-buttons"), S = document.getElementById(
...
async function U() {
  const e = await V(), t = e.currentSize.height > e.mainContentSize.height, i = e.currentSize.width > e.mainContentSize.width, n = e.naturalSize.height > e.currentSize.height, o = e.naturalSize.width > e.currentSize.width;
  return {
    isScalable: n && o,
    isInvertible: t && i
  };
...
const ne = (e) => {
  e ? (u.setAttribute("disabled", ""), u.style.cursor = "default", [...u.children][0].style.fill = "var(--viewover-icon-color-disable)") : (u.removeAttribute("disabled"), u.style.cursor = "pointer", [...u.children][0].style.fill = "var(--viewover-icon-color)");
}, oe = (e) => {
  e ? (p.setAttribute("disabled", ""), p.style.cursor = "default", [...p.children][0].style.fill = "var(--viewover-icon-color-disable)") : (p.removeAttribute("disabled"), p.style.cursor = "pointer", [...p.children][0].style.fill = "var(--viewover-icon-color)");
}, Ie = () => {
  u.classList.contains("viewover__zoom-button_visible") || (u.classList.add("viewover__zoom-button_visible"), p.classList.add("viewover__zoom-button_visible")), ne(!0), oe(!0);
}, Z = async () => {
  await V();
  const e = await U();
  e.isScalable && ne(!1), e.isInvertible && oe(!1);
}, Ve = () => {
  u.classList.contains("viewover__zoom-button_visible") && (u.classList.remove("viewover__zoom-button_visible"), p.classList.remove("viewover__zoom-button_visible"));
}, Ae = async () => {
  const e = await V(), t = await U(), i = e.currentSize.width, n = e.currentSize.height, o = e.naturalSize.width, r = e.naturalSize.height;
  ...
  if (e.ctrlKey) {
    e.preventDefault();
    const t = await V(), i = await U(), n = t.currentSize.width, o = t.currentSize.height, r = t.naturalSize.width, s = t.naturalSize.height;
...
async function z(e, t) {
  L.children.length !== 0 && Ce(), He(e, t), L.appendChild(a.element), (e === "video" || e === "iframe") && Ve(), e === "img" && l.enableZoom == !0 && (a.element.removeEventListener("wheel", te), a.element.addEventListener("wheel", te), Ie(), Z()), e === "video" && a.element.load(), a.element.style.transition = "var(--viewover-media-transition)";
}
const A = (e, t) => {
...
function Ne(e, t) {
  e.length === 1 && (e.includes("img") && A(t, "img"), e.includes("iframe") && A(t, "iframe"), e.includes("video") && A(t, "video")), e.length !== 1 && A(t, t.localName);
}
const We = () => {
...
const E = (e) => {
  ...
  v.offsetWidth < v.scrollWidth ? h.style.setProperty(
...
function f(e, t) {
  e.classList.remove("button_hidden"), e.classList.add("button_visible"), t.classList.remove("button_visible"), t.classList.add("button_hidden");
}
const le = (e, t) => {
...
async function Ue(e, t) {
  t ? (h.style.setProperty(
    "--thumbnails-track-height",
    ee
  ), f(S, M)) : f(M, S);
  for (let i = 0; i < e.length; i++) {
    const n = e[i];
    n.localName === "img" && (g = await D(n), b = g.querySelector(".viewover__thumbnail"), b.setAttribute(
      "data-viewover-thumbnail-src",
      n.src
    ), b.setAttribute("data-viewover-thumbnail-type", "img")), n.localName === "video" && (g = await D(n), b = g.querySelector(".viewover__thumbnail"), b.setAttribute(
      "data-viewover-thumbnail-src",
      n.currentSrc
    ), b.setAttribute("data-viewover-thumbnail-type", "video")), n.localName === "iframe" && (g = await D(n), b = g.querySelector(".viewover__thumbnail"), b.setAttribute(
      "data-viewover-thumbnail-src",
      n.src
    ), b.setAttribute("data-viewover-thumbnail-type", "iframe")), v.appendChild(g);
  }
}
function K(e) {
...
function se() {
  ...
  ), h.style.setProperty("--thumbnails-track-height", "0px"), h.style.setProperty(
    "--viewover-thumbnails-track-margin",
    "0px"
  )) : (f(S, M), h.style.setProperty(
    "--thumbnails-track-height",
    ee
  ), h.style.setProperty(
    "--viewover-thumbnails-track-margin",
...
let q;
const X = () => {
  clearTimeout(q), m.style.visibility = "visible", m.style.opacity = "1";
}, T = () => {
...
function me() {
  [...L.children].forEach((t) => {
    t.classList.contains("button") || t.remove();
  }), Re(v), qe(), document.removeEventListener("keydown", ie), x.removeEventListener("wheel", $e), L.removeEventListener("dblclick", I), a.element.removeEventListener("wheel", te), f(M, S), v.classList.remove(
    "viewover__thumbnails-track_visible"
  ), h.style.setProperty("--thumbnails-track-height", "0px"), h.style.setProperty(
    "--viewover-thumbnails-track-margin",
    "0px"
...
function De(e) {
  e.zoom == !0 && (u.addEventListener("click", Ae), p.addEventListener("click", Te)), Se.onclick = () => {
    Ye();
  }, document.addEventListener("keyup", (t) => {
    const i = B.hasAttribute("open");
    t.key === "Escape" && me(), t.key === "Enter" && i && e.fullscreen == !0 && I();
  }), document.addEventListener("fullscreenchange", () => {
    W.classList.contains("button_hidden") ? (f(W, Q), ae.removeEventListener("mousemove", T), m.removeEventListener("mouseenter", X), m.removeEventListener("mouseleave", T), X()) : (f(Q, W), ae.addEventListener("mousemove", T), m.addEventListener("mouseenter", X), m.addEventListener("mouseleave", T));
  });
}
const l = {};
let ie;
const ce = async (e, t) => {
  ...
  if (l.enableZoom = t.zoom, Ne(o, i), Ee.onclick = () => {
    ...
    R(t);
  }, ie = (r) => Oe(r, t), document.addEventListener("keydown", ie), c.init(), S.addEventListener("click", se), M.addEventListener("click", se), t.fullscreen == !0 ? (W.addEventListener("click", I), Q.addEventListener("click", I), L.addEventListener("dblclick", I)) : Me.classList.add("viewover__fullscreen-buttons_hidden"), We(), B.showModal(), De(t), t.thumbnails == !0) {
    ...
    Be.classList.add("viewover__thumbnails-buttons_hidden");
...
_ = new WeakMap(), O = new WeakSet(), we = function() {
  return {
    fullscreen: this.options.fullscreen || C(this, _).fullscreen,
    thumbnails: this.options.thumbnails || C(this, _).thumbnails,
    zoom: this.options.zoom || C(this, _).zoom
  };
}, $ = new WeakSet(), be = function() {
  this.options == null ? this.updatedOptions = C(this, _) : this.updatedOptions = H(this, O, we).call(this);
}, P = new WeakSet(), pe = function() {
  const t = document.createElement("style");
  t.textContent = Ge, document.head.append(t);
}, j = new WeakSet(), ge = function() {
  H(this, $, be).call(this), Xe(this.updatedOptions);
};
export {
  Qe as default
};
